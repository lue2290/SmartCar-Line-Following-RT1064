...
void Set_Motor(int16 l_duty, int16 r_duty)
{
    ...
    if (r_duty > 0)
    {
        r_duty = Limit_Min_Max(r_duty, 0, 8000); // 8000
        pwm_set_duty(PWM1_MODULE3_CHA_D0, 0);
        pwm_set_duty(PWM1_MODULE3_CHB_D1, r_duty);
    }
    // 右电机反转
    else
    {
        r_duty = Limit_Min_Max(-r_duty, 0, 8000);
        pwm_set_duty(PWM1_MODULE3_CHA_D0, r_duty);
    ...
    if (l_duty > 0)
    {
        ...
        pwm_set_duty(PWM2_MODULE3_CHA_D2, l_duty);
    }
    // 左电机反转
    else
    {
        l_duty = Limit_Min_Max(-l_duty, 0, 8000);
        pwm_set_duty(PWM2_MODULE3_CHB_D3, l_duty);
...
{
    // 获取左编码器脉冲
    l_encoder_pulse = -qtimer_quad_get(QTIMER_1, QTIMER1_TIMER2_C2); // 正转脉冲为负
    qtimer_quad_clear(QTIMER_1, QTIMER1_TIMER2_C2);

    // 获取右编码器脉冲
    r_encoder_pulse = qtimer_quad_get(QTIMER_1, QTIMER1_TIMER0_C0);
    qtimer_quad_clear(QTIMER_1, QTIMER1_TIMER0_C0);
    // 获取左编码器脉冲
    l_encoder_pulse = -encoder_get_count(QTIMER1_ENCOEDER2); // 正转脉冲为负
    encoder_clear_count(QTIMER1_ENCOEDER2);

    // 获取右编码器脉冲
    r_encoder_pulse = encoder_get_count(QTIMER1_ENCOEDER1);
    encoder_clear_count(QTIMER1_ENCOEDER1);

    l_speed_now = l_encoder_pulse;
    r_speed_now = r_encoder_pulse;
    // tft180_show_int(0, 0, l_speed_now, 3);
    // tft180_show_int(0, 50, r_speed_now, 3);
}
...
void L_speed_pid(struct PID *sptr)
{
    ...
    sptr->error = L_speed_error1(); // 左轮速度偏差计算
...
void R_speed_pid(struct PID *sptr)
{
    ...
    sptr->error = R_speed_error1(); // 右轮速度偏差计算
    sptr->out += sptr->kp * (sptr->error - sptr->last_error) + sptr->ki * sptr->error;
    sptr->last_error = sptr->error;
}
