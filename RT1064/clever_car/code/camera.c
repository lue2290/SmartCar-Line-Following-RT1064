...
void Camera_Display(void)
{
    ...
    //threshold = GetOSTUThreshold(image_yuanshi,0,MT9V03X_H-1,0,MT9V03X_W-1);
...
void Element_Test(void)
{
    ...
    {
        check_cheku(55, 50, 2); // 车库检测45  40                55 50
    }
...
//        }
        // 检测十字
        else if (length > 60 && r_lose_value > 15 && l_lose_value > 15) // 20
        {
            //
            //
//			gpio_set_level(BEEP, GPIO_HIGH);
...
//    if (flag.open_check_ku)
//    {
//        check_cheku(45, 40, 2); // 车库检测45  40    B区：75,70,  A区：53 48
//    }
//    if (!road_type.Barn_r_in && !road_type.Barn_l_in)
//    {
//        // 判断左右车库
//        if (flag_starting_line)
//        {
...
//    if (!road_type.Fork && !road_type.LeftCirque && !road_type.RightCirque && !road_type.L_Cross && !road_type.R_Cross && !road_type.Cross && huandao_count == 0)
//    {
//        /*
//        //判断左斜十字和左圆环
//        else if(length > 60 && l_lose_value >30 && r_lose_value <3 && fabs(r_line_qulv) <4 && !Tututu(2))//
...
//        }*/
//        // 右斜十字和右圆环
//        if (length > 100 && r_lose_value > 40 && l_lose_value < 3 && abs(l_line_qulv) < 4 && !Tututu(1)) //
//        {
//            //
//            if (Cirque_or_Cross(2, r_losemax) > 40 && road_type.RightCirque == 0 && road_type.R_Cross == 0 && road_type.Fork == 0) // 左侧丢线位置y的下方有较多白点,判断为圆环
//            {
...
//        speed_type.straight = speed_type.bend = 3300;
//    }
//    else
//    {

//        speed_type.straight = speed_type.bend = 4000;
//    }
//}

//void Element_Test1(void)
//{
//    l_line_qulv = 1000 * Process_Curvity(l_line_x[MT9V03X_H - 10], MT9V03X_H - 10, l_line_x[MT9V03X_H - 25], MT9V03X_H - 25, l_line_x[MT9V03X_H - 40], MT9V03X_H - 40);
//    r_line_qulv = 1000 * Process_Curvity(r_line_x[MT9V03X_H - 10], MT9V03X_H - 10, r_line_x[MT9V03X_H - 25], MT9V03X_H - 25, r_line_x[MT9V03X_H - 40], MT9V03X_H - 40);
...
//        }
//    }
//    else if (!road_type.LeftCirque && !road_type.RightCirque && !road_type.L_Cross && !road_type.R_Cross)
//    {
//        //
//        // 判断左斜十字和左圆环
//        if (length > 90 && l_lose_value > 30 && r_lose_value < 20 && fabs(r_line_qulv) < 40 && !Tututu(2)) //
//        {
...
//    {
//        // 检测十字
//        if (length > 60 && r_lose_value > 20 && l_lose_value > 20) // 20
//        {
...
//    // tft180_show_int(0, 80, cross_left[0]  , 3);
//    // tft180_show_int(0, 100, cross_left[1] , 3);

//    Bianxian_guaidian_num(2, 80, 10);
//    // Bianxian_guaidian_num(2, 100, 10 );
//    // tft180_show_int(0, 40, r_guaidain_x1  , 3);
//    // tft180_show_int(0, 60, r_guaidain_x2 , 3);
//    tft180_show_int(0, 80, r_guaidain_x2, 3);
//    tft180_show_int(0, 100, r_guaidain_x1, 3);
//    // Diuxian_weizhi_test(1, MT9V03X_H -1, MT9V03X_H -30);
//    // diuxian_l=diuxian_hang;

//    if (r_guaidain_x1 != 0 && r_guaidain_x2 != 0) // 左侧上下拐点都存在
//    {                                             // 十字一阶段

//        La_zhixian(l_guaidain_x1, l_guaidain_y1, l_guaidain_x2 + 10, l_guaidain_y2 + 10, l_line_x); // 输入两个点的横纵坐标
//        // tft180_draw_point(l_line_x[cross_left[1]], cross_left[1], RGB565_RED);
//        road_type.LeftCirque = 1;
//        // huandao_stage=3;
//        road_type.straight = 0;
//        road_type.bend = 0;
//    }
//    else if (r_guaidain_x1 == 0 && r_guaidain_x2 != 0) // 只能搜到左上点（十字中）
//    {                                                  // 十字二阶段
//        //
//        Regression(1, l_guaidain_y2 - 10, l_guaidain_y2 - 20);
//        Hua_Xian(1, l_guaidain_y2 - 20, MT9V03X_H - 3, parameterB, parameterA);
//        road_type.LeftCirque = 1;
//        // huandao_stage=4;
//        road_type.straight = 0;
//        road_type.bend = 0;
...
// 二值化
uint8 Threshold;               // 阈值
uint8 Threshold_static = 200;  // 阈值静态下限160 220  230            白天：200   晚上：230       A:200
uint16 Threshold_detach = 300; // 阳光算法分割阈值(光强越强,该值越大)   白天：300
...
void Blacking()
{
    ...
    tft180_displayimage03x(image_01[0], 160, 128); // 需要注意 直接显示 188*120 分辨率是显示不下的 会直接进入断言报错
...
{ // garage_direction=1为左车库，garage_direction=2为右车库
    ...
    for (uint8 height = start_point; height >= end_point; height--) // 判断斑马线
    {
        ...
        for (uint8 width_l = MT9V03X_W / 2, width_r = MT9V03X_W / 2; width_l >= 1 && width_r < MT9V03X_W - 2; width_l--, width_r++)
        {
            ...
            {
                if (cursor_l >= qiangdu && cursor_l <= qiangdu + 8) // 这个范围是不是给小了(4,8) > (4,12)
                {
                    black_blocks_l++;
                    X2 = black_blocks_l;
                    cursor_l = 0;
        ...
        if ((black_blocks_l + black_blocks_r) >= 4 && (black_blocks_l + black_blocks_r) <= 10)
        {
            times++;
        }
...
//            }
//        }
//        for (uint8 i = y2 - 10; i > 10; i--) // y1 -10
//        {
//            if (image_01[i][0] == 0 && image_01[i - 1][0] == 0 && image_01[i - 2][0] == 0) // 找到上方黑点
//            {
//                y2 = i;
...
//        for (uint8 i = y2; y2 > 10; i--)
//        {
//            if ((r_line_x[y2 - 2] - r_line_x[y2] < 10) && (r_line_x[y2 - 4] - r_line_x[y2] < 20) && (r_line_x[y2 - 6] - r_line_x[y2] < 30)) // 利用圆环横坐标增加没有十字快的特征（猜想）
//            {
//				gpio_set_level(BEEP, GPIO_HIGH);
//			    system_delay_ms(100);
//				gpio_set_level(BEEP, GPIO_LOW);
...
uint8 Bianxian_guaidian_num(uint8 type, uint8 startline, uint8 endline)
{
    ...
    r_guaidain_x2 = 0;
    r_guaidain_y2 = 0;
    Diuxian_weizhi_test(2, MT9V03X_H - 30, MT9V03X_H - 60);

    // 左边线拐点
    if (type == 1)
    {
        if (diuxian_hang < 15) // 左下拐点存在if(l_line_x[MT9V03X_H -10] !=0)
        {
            // 第一个拐点
            for (uint8 y = startline; y > endline; y--)
            {
                if (fabs(l_line_x[y] - l_line_x[y + 1]) < 4 && (l_line_x[y] - l_line_x[y - 3] > 10))
                {
    ...
    if (type == 2)
    {
        if (diuxian_hang < 10) // 右下拐点存在
        {
            // 第一个拐点
            for (uint8 y = startline; y > endline; y--)
            {
                if (fabs(r_line_x[y] - r_line_x[y + 1]) < 4 && (r_line_x[y] - r_line_x[y - 3] < -10)) // 这个条件要更加严格一点
                {
...
void Handle_Left_Cirque()
{
    ...
    case 4:
        annulus_s1 += Distance_Measure();
        if (annulus_s1 > 30500) // 编码器积分大于一个值，进入环岛里面（这个值可以给小一些，车身路径不好）
        {
    ...
    case 7:
        annulus_s1 += Distance_Measure();
        if (annulus_s1 > 7500) // 积分一段距离并补线环岛处理结束(所有标志位清零)
        {
            road_type.LeftCirque = 0;
            huandao_stage = 1;
            annulus_s1 = 0;
    ...
    {
        for (uint8 y = MT9V03X_H - 30; y >= MT9V03X_H - 100; y--) // 只有80行的赛道宽度，够用了
        {
    ...
    else if (huandao_stage == 4) // 赛道宽度补线
    {
        for (uint8 y = MT9V03X_H - 30; y >= MT9V03X_H - 40; y--) // 只有80行的赛道宽度，够用了
        {
            // r_line_x[y] = l_line_x[y] + kuandu_saidao[MT9V03X_H-1 -y-20];   //随便给的一个偏移量，给小一点会内切
            //            l_line_x[y] = l_line_x[y] ;
            for (uint8 y = MT9V03X_H - 10; y >= l_guaidain_y2; y--) // 只有80行的赛道宽度，够用了
            {
...
void Handle_Right_Cirque()
{
    ...
    case 3:
        Diuxian_weizhi_test(2, MT9V03X_H - 20, MT9V03X_H - 40);
        if (diuxian_hang >= 15 && huandao_stage == 3)
        {
    ...
    case 4:
        annulus_s1 += Distance_Measure();
        if (annulus_s1 > 50000) // 编码器积分大于一个值，进入环岛里面（这个值可以给小一些，车身路径不好）
        {
            huandao_stage = 5; // 进入圆环内
            annulus_s1 = 0;
        }
    ...
    case 5:
        ...
        annulus_s1 += Distance_Measure();
        Diuxian_weizhi_test(1, MT9V03X_H - 1, MT9V03X_H - 60);
        if (annulus_s1 > 80000)
        {
            if (diuxian_hang >= 50)
            {
                Diuxian_weizhi_test(2, MT9V03X_H - 1, MT9V03X_H - 60);
                if (diuxian_hang >= 50)
                {
                    huandao_stage = 6;
                    annulus_s1 = 0;
    ...
    case 7:
        annulus_s1 += Distance_Measure();
        if (annulus_s1 > 50000) // 积分一段距离并补线环岛处理结束(所有标志位清零)
        {
            road_type.RightCirque = 0;
    ...
    {
        for(uint8 y =MT9V03X_H -1; y >=MT9V03X_H -40; y--)  //只有80行的赛道宽度，够用了
        {
            r_line_x[y] = l_line_x[y] + kuandu_saidao[MT9V03X_H-1 -y] + 10;
            //l_line_x[y] = l_line_x[y] + 10;
        }
    }*/
    else if ((huandao_stage == 2))
    {
        for (uint8 y = MT9V03X_H - 1; y >= MT9V03X_H - 70; y--) // 只有80行的赛道宽度，够用了
        {
            r_line_x[y] = l_line_x[y] + kuandu_saidao[MT9V03X_H - 1 - y] + 70;
            // l_line_x[y] = l_line_x[y] + 10;
        }
    }
    else if (huandao_stage == 4) // 赛道宽度补线
    {
        for (uint8 y = MT9V03X_H - 30; y >= MT9V03X_H - 40; y--) // 只有80行的赛道宽度，够用了
        {
            l_line_x[y] = r_line_x[y] - kuandu_saidao[MT9V03X_H - 1 - y]; // 随便给的一个偏移量，给小一点会内切
            r_line_x[y] = r_line_x[y] + 5;
        }
    }
    else if (huandao_stage == 5) // 在环岛
    ...
    else if (huandao_stage == 6) // 出环岛
    {
        ...
        for (uint8 y = MT9V03X_H - 1; y > 20; y--) // 20
        {
            // r_line_x[y] = l_line_x[y] + kuandu_saidao[MT9V03X_H-1 -y];     //车辆路径偏右,为了准确识别环岛,采用偏移量使车身更加贴近左侧
            l_line_x[y] = r_line_x[y] - kuandu_saidao[MT9V03X_H - 1 - y] - 20;
        }
    }
    else if (huandao_stage == 7)
    {
        for (uint8 y = MT9V03X_H - 1; y > 20; y--)
        {
            r_line_x[y] = l_line_x[y] + kuandu_saidao[MT9V03X_H - 1 - y] + 40; // 车辆路径偏右,为了准确识别环岛,采用偏移量使车身更加贴近左侧
            // l_line_x[y] = r_line_x[y] - kuandu_saidao[MT9V03X_H-1 -y];
        }
    }
}
...
void Check_Cross_Guaidian(uint8 type) // 找十字拐点
{
    ...
    if (type == 1)
    {
        if (l_line_x[MT9V03X_H - 1] != 0) // 左下拐点存在
        {
            ...
            {
                if (abs(l_line_x[y1] - l_line_x[y1 + 1]) < 4 && (l_line_x[y1] - l_line_x[y1 - 3]) > 8 && l_line_x[y1 - 3] == 0)
                {
                    cross_left[0] = y1;
                    break;
                }
            ...
            {
                for (uint8 y2 = cross_left[0] - 10; y2 >= 2; y2--)
                {
                    if (abs(l_line_x[y2] - l_line_x[y2 - 1]) < 4 && (l_line_x[y2] - l_line_x[y2 + 3]) > 8 && l_line_x[y2 + 3] == 0)
                    {
                        cross_left[1] = y2;
                        break;
                    }
        ...
        else // 左下拐点不存在
        {
            ...
            {
                if (abs(l_line_x[y] - l_line_x[y - 1]) < 4 && (l_line_x[y] - l_line_x[y + 3]) > 8 && l_line_x[y + 3] == 0)
                {
                    cross_left[0] = 0;
                    cross_left[1] = y;
                    break;
    ...
    else if (type == 2)
    {
        if (r_line_x[MT9V03X_H - 1] != MT9V03X_W - 1) // 右下拐点存在
        {
            for (uint8 y1 = MT9V03X_H - 1; y1 >= 2; y1--)
            {
                if (abs(r_line_x[y1] - r_line_x[y1 + 1]) < 4 && (r_line_x[y1] - r_line_x[y1 - 3]) < -8 && r_line_x[y1 - 3] == MT9V03X_W - 1)
                {
                    cross_right[0] = y1;
                    break;
                }
            ...
            {
                for (uint8 y2 = cross_right[0] - 10; y2 >= 2; y2--)
                {
                    if (abs(r_line_x[y2] - r_line_x[y2 - 1]) < 4 && (r_line_x[y2] - r_line_x[y2 + 3]) < -8 && r_line_x[y2 + 3] == MT9V03X_W - 1)
                    {
                        cross_right[1] = y2;
                        break;
                    }
        ...
        else // 右下拐点不存在
        {
            for (uint8 y = MT9V03X_H - 1; y >= 2; y--)
            {
                if (fabs(r_line_x[y] - r_line_x[y - 1]) < 4 && r_line_x[y] - r_line_x[y + 3] < -8 && r_line_x[y + 3] == MT9V03X_W - 1)
                {
                    cross_right[0] = 0;
                    cross_right[1] = y;
                    break;
...
void Check_Cross_Guaidian1(uint8 type) // 找十字拐点
{
    cross_left[0] = cross_left[1] = cross_right[0] = cross_right[1] = 0;
    ...
    if (type == 1)
    {
        if (image_01[MT9V03X_H - 1][4] == 0) // 左下拐点存在
        {
            ...
            if (cross_left[0] != 0) // 左下拐点找到了,接着找左上拐点
            {
                ...
                {
                    if (fabs(l_line_x[y2] - l_line_x[y2 - 1]) < 4 && l_line_x[y2] - l_line_x[y2 - 5] < -60 && l_line_x[y2] == 0)
                    {
                        cross_left[1] = y2 - 5 - 5;
                        break;
                    }
        ...
        else // 左下拐点不存在
        {
            for (uint8 y = MT9V03X_H - 1; y >= 2; y--)
            {
                if (fabs(l_line_x[y] - l_line_x[y - 1]) < 4 && (l_line_x[y] - l_line_x[y - 10]) < -60 && l_line_x[y] == 0)
                {
                    cross_left[0] = 0;
                    cross_left[1] = y - 5 - 5;
                    break;
...
{ // 1左入库,2右入库
    if (type == 1)
    {
        if (pass_barn == 1) // 第一次经过库
        {
            ...
            if (close_check_ku_s > 2000) // 积分一段距离，打开库检测标志位
            {
                pass_barn++;
                road_type.Barn_l_in = 0;
...
void Handle_Barn_in1(uint8 type)
{ // 1左入库,2右入库
    if (type == 1)
    {
        if (pass_barn == 1) // 第一次经过库
...
void get_binImage(uint8 thres)
{
    for (int row = 0; row < MT9V03X_H; row++)
    {
        ...
        {
            if (image_yuanshi[row][col] < thres)
            {
                image_yuanshi[row][col] = 0; // 小于阈值赋值为0，显示为黑色
            }
            else
                image_yuanshi[row][col] = 1; // 大于阈值赋值为1，显示为白色
        }
    }
}
